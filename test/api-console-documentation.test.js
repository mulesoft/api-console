    ...
    describe(label, () => {
      ...
      describe('Summary section', () => {
        ...
        it(`should render basic summary documentation`, async () => {
          ...
          assert.equal(summaryShadowRoot.querySelector('.api-title').innerText.trim(), 'API title: Google Drive')

          await waitUntil(() => Boolean(summaryShadowRoot.querySelector('.inline-description.version')));
          assert.equal(summaryShadowRoot.querySelector('.inline-description.version').innerText.trim(), 'Version: v2')

          const url = summaryShadowRoot.querySelector('api-url').shadowRoot;
          await waitUntil(() => Boolean(url.querySelector('.url-area > .url-value')));
          assert.equal(url.querySelector('.url-area > .url-value').innerText.trim(), 'https://www.googleapis.com/drive/{version}')

          await waitUntil(() => Boolean(summaryShadowRoot.querySelector('.marked-description')));
          assert.equal(summaryShadowRoot.querySelector('.marked-description').innerText.trim(), 'Google Drive API')
        });
